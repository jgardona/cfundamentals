...
using SymPy, Latexify, LaTeXStrings, Markdown, PlutoUI, Plots, PlotThemes
...
O segredo para se trabalhar com circuito RC sem fonte é determinar:
- A tensão inicial $v(0)=V_o$ no capacitor.
...
let
	...
	iₜ = latexraw(iₜ)
	mdtext = """
	``i(t)`` para ``t \\gt 0`` é: ``$iₜ``
	"""
	Markdown.parse(mdtext)
end

# ╔═╡ 652c7604-3d93-4b1a-aae7-81afec0971c7
md"""
### Exemplo 7.5
No circuito indicado na figura 7.9, encontre $i_0$, $v_0$ e $i$ durante todo o tempo, supondo que a chave fora aberta por um longo período.
"""

# ╔═╡ a52f339b-7b11-4489-a592-eba50e055799
let
	# Para t < 0, a chave se encontra aberta. Como o indutor atua como
	# um curto circuito em CC, o resistor de 6Ω está curto circuitado, de modo
	# a termos o circuito mostrado na figura 7.20a. Logo, i₀=0.
	...
	# Para t > 0, a chave está fechada e, portanto, a fonte de tensão está curto
	# circuitada. Agora temos um circuito RL sem fonte. Nos terminais do indutor:
	req = rth = parallel(r₂, r₃)
	# Calcula τ (constante de tempo).
	tau = l₁/req
	iₜᵣₗ = i₀*exp(-t/τ)
	iₜᵣₗ = subs(iₜᵣₗ, τ=>tau)
	v₀ᵣₗ = -l₁*diff(iₜᵣₗ)
	iₒ = v₀ᵣₗ/r₃

	mdtext = """
	``v_o`` para ``t \\gt 0`` é: ``$(latexraw(v₀ᵣₗ))``
	
	``i(t)`` para ``t \\gt 0`` é: ``$(latexraw(iₜᵣₗ))``

	``i_o`` para ``t \\gt 0`` é: ``$(latexraw(iₒ))``
	"""
	Markdown.parse(mdtext)
end

# ╔═╡ 03e48e04-77b3-4dfe-8c09-f091e7a931ed
md"""
### Problema Prático 7.5
Determine $i$, $i_o$ e $v_o$ para todo $t$ no circuito mostrado na figura 7.22. Suponha que a chave foi fechada por um longo período. Deve-se observar que abrir uma chave em série com uma fonte de corrente ideal cria uma tensão infinita nos terminais da fonte de corrente. É óbvio que essa situação é impossível. Para haver resolução do problema, podemos colocar um resistor shunt em paralelo com a fonte (que agora a transforma em uma fonte de tensão em série com um resistor). Em circuitos mais práticos, dispositivos que atuam como fontes de corrente são, em sua maioria, circuitos eletrônicos, que permitirão que a fonte atue como uma fonte de corrente ideal em seu intervalo operacional, porém a limitará em termos de tensão quando o resistor de carga for muito grande (como em um circuito aberto).
"""

# ╔═╡ f90a2597-c150-407a-b721-44c6dc8cdf9a
let
	r₁ = 3
	r₂ = 4
	r₃ = 2
...
# ╔═╡ 1b4a124b-15d0-4eaf-951f-aa6f9c7c4f33
md"""
## Funções de Singularidade
**Funções de singularidade**  são funções que são _descontínuas_ ou então que
apresentam _derivadas descontínuas_.

### Função de Degrau Unitário
A **função de degrau unitário** $u(t)$ é 0 para valores negativos de $t$ e 1 para valores positivos de $t$.
...
# ╔═╡ 1f20f9cc-5259-4738-a9e1-68e5c11e2b0c
let
    @syms t t0
    # Implementação da função de degrau unitário.
    u = sympy.Piecewise(
        (0, Lt(t, 0)),
        (1, Gt(t, 0))
    )
    ...
    p1 = plot(u, xlabel="Time(s)", ylabel="Amplitude (V)", legend=:none,
		 lw=3, size=(400, 300), title="Degrau Unitário", c=:tomato)
	
	# A função degrau unitário pode sofrer um atraso alterando-se t por t - t₀.
	u = sympy.Piecewise(
		(0, Lt(t, t0)),
		(1, Gt(t, t0))
	)
	...
	u = subs(u, t0=>2)
	p2 = plot(u, xlabel="Time(s)", ylabel="Amplitude (V)", legend=:none,
		 lw=3, size=(400, 300), title="Retardada em 2s", c=:green)

	# A função degrau unitário pode sofrer adiantamento alterando-se t por t + t0.
	u = sympy.Piecewise(
		(0, Lt(t, -t0)),
		(1, Gt(t, -t0))
	)
	...
	u = subs(u, t0=>2)
	p3 = plot(u, xlabel="Time(s)", ylabel="Amplitude (V)", legend=:none,
		 lw=3, size=(400, 300), title="Adiantada em 2s", c=:lightblue)

	plot(p1, p2, p3, layout=(1, 3), size=(900, 400), margin=5Plots.mm)
end

# ╔═╡ 66c34b83-af1d-49cd-9c95-49343f7df77d
md"""
### Função de Impulso Unitário
Consiguimos a função de impulto unitário diferenciando $u(t)$.
```math
\delta(t) = \frac{d}{dt} u(t) = 
\begin{cases}
0 & \text{se } t < 0 \\
indefinido & \text{se } t = 0 \\
0 & \text{se } t > 0
\end{cases}
```

- A **função de impulso unitário** $\delta(t)$ é zero para todos os valores exceto em $t=0$, onde é indefinida.
"""

# ╔═╡ 19250561-d217-4f80-ae1a-2712c4f3c322
let
	@syms t t0
    # Implementação da função de degrau unitário.
    u = sympy.Piecewise(
	...
	@info u
	δ = diff(u)
	# A função de impulso não é plotável dessa forma.
	@info δ
end

# ╔═╡ 10d49af7-d1af-4334-8783-f41a44281331
md"""
### Função Rampa Unitária
Conseguimos a função rampa unitária $r(t)$ integrando a função degrau unitário $u(t)$.
...
- A **função rampa unitária** é zero para valores negativos de $t$ e apresenta uma inclinação unitária para valores positivos de $t$.
"""

# ╔═╡ 8f00757c-f1f8-4442-8fea-d8aeaaee66a6
let
	@syms t t₀
    # Implementação da função de degrau unitário.
    u = sympy.Piecewise(
	...
	@info r
	p1 = plot(r,
			 legend=false, lw=3, c=:tomato,
			 xlabel="Tempo (s)", ylabel="Amplitude (V)",
			 title="Função Rampa Unitária")
	# Alterando t por t - t₀, retardamos a função em t₀.
	r₁ = subs(r, t=> t-t₀)
	r₁ = subs(r₁, t₀=> 2)
	@info r₁
	p2 = plot(r₁,
			 lw=3, xlabel="Tempo (s)", ylabel="Amplitude (V)",
			 title="Retardada em 2 s", c=:lightblue, legend=:none)
	r₂ = subs(r, t=> t+t₀)
	r₂ = subs(r₂, t₀=> 2)
	@info r₂
	p3 = plot(r₂,
			 lw=3, xlabel="Tempo (s)", ylabel="Amplitude (V)",
			 title="Avançada em 2 s", c=:green, legend=:none) 
	plot(p1, p2, p3, layout=(1, 3), size=(900, 400), margin=5Plots.mm)
end

# ╔═╡ 9fd3e747-a027-4494-9eb6-8f12f6f9805e
md"""
### Exemplo 7.6
...
# ╔═╡ 575adc63-acf7-422c-a33d-47aa75147516
let
	@syms t t₀
	...
	p2 = plot(0:8, f;
		 ...
		 title=L"Função de Impulso Unitário $\delta(t)$"
		)
	plot(p1, p2, layout=(1, 2), size=(800, 300), margin=5Plots.mm)
	
end

# ╔═╡ 34d2d248-c6aa-4202-9be0-354391d58580
md"""
...
manifest_format = "2.0"
project_hash = "4b94779aeb746346f0d4fa5ea9965bf56761384f"
